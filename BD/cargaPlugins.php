...
$query_total = "SELECT count(`resumenPlugins`) FROM `atributos`";
$total_reg = $conn->query($query_total);
$total_reg = $total_reg->fetch_all();

//Cogemos el resumePlugins de quien ha hecho la petición
$stmt = $conn->prepare("SELECT `resumenPlugins` FROM `atributos` WHERE id =?");
...
if (is_null($resumenPlugins)) {
    $stmt = $conn->prepare("SELECT COUNT(DISTINCT `resumenPlugins`) FROM `atributos` WHERE `resumenPlugins` =? is null");
    ...
    $stmt = $conn->prepare("SELECT COUNT(DISTINCT `resumenPlugins`) FROM `atributos` WHERE `resumenPlugins` =?");
    $stmt->bind_param("s", $resumenPlugins);
    $stmt->execute();
    $stmt -> store_result();
    $stmt -> bind_result($count);
