...
if (is_null($resumenPlugins)) {
    $stmt = $conn->prepare("SELECT COUNT(DISTINCT `resumenPlugins`) FROM `resultados` WHERE `resumenPlugins` =? is null");
    ...
    $stmt = $conn->prepare("SELECT COUNT(DISTINCT `resumenPlugins`) FROM `resultados` WHERE `resumenPlugins` =?");
    $stmt->bind_param("s", $resumenPlugins);
    $stmt->execute();
    $stmt -> store_result();
    $stmt -> bind_result($count);
